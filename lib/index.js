...
var fetcher = isNode ? require('node-fetch') : fetch;
...
function parse() {
  ...
  Parse.login = function (creds) {
    ...
    fetcher(base + '/apps?id=' + Parse.session.client).then(function (res) {
...
function topapps(session) {
  return fetcher(base + '/topapps', withAuth(session)).then(function (res) {
    return res.json();
  });
}

function size(session) {
  return fetcher(base + '/size', withAuth(session)).then(function (res) {
    return res.json();
  });
}
